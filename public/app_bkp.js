...
async function carregarListaEquipamentos() {
  ...
  document.getElementById('lista_equipamentos').innerHTML = `
    ...
    ${lista.map(e => `<tr>
      ...
      <td>
        <button class="action" onclick='editarEquipamento(${JSON.stringify(e)})'>Editar</button>
  ...
  document.getElementById('lista_epis').innerHTML = `
    ...
    ${lista.map(e => `<tr>
      ...
      <td>
        ...
        <button class="action" onclick='editarEpi(${JSON.stringify(e)})'>Editar</button>
  ...
  document.getElementById('lista_consumiveis').innerHTML = `
    ...
    ${lista.map(c => `<tr>
      ...
      <td>
        ...
        <button class="action" onclick='editarConsumivel(${JSON.stringify(c)})'>Editar</button>
...
async function carregarListaProdutos() {
  ...
  document.getElementById('lista_produtos').innerHTML = `
    <table><tr><th>Código</th><th>Nome</th><th>Fornecedor</th><th>Unidade</th><th>Estoque</th><th>Mín</th><th></th></tr>
    ${lista.map(p => `<tr>
      <td>${p.codigo}</td><td>${p.nome}</td><td>${p.fornecedor_nome || '-'}</td><td>${p.unidade_sigla || '-'}</td>
      ...
      <td>
        ...
        <button class="action" onclick='editarProduto(${JSON.stringify(p)})'>Editar</button>
...
async function carregarEmprestimosAtivos() {
  ...
  document.getElementById('lista_emprestimos').innerHTML = `
    <table><tr><th>Tag</th><th>Equipamento</th><th>Funcionário</th><th>Data</th><th>Observação</th><th></th></tr>
    ${lista.map(e => `<tr>
...
async function buscarFicha() {
  ...
  document.getElementById('ficha_resultado').innerHTML = `
    ...
    <div class="card"><h4>Equipamentos</h4>
      <table><tr><th>Equipamento</th><th>Empréstimo</th><th>Devolução</th><th>Observação</th></tr>
      ${data.equipamentos.map(e => `<tr><td>${e.equipamento_nome}</td><td>${new Date(e.data_emprestimo).toLocaleString()}</td><td>${e.data_devolucao ? new Date(e.data_devolucao).toLocaleString() : '-'}</td><td>${e.observacao || '-'}</td></tr>`).join('')}
      </table>
    </div>
